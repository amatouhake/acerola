...
function generate() {
  ...
  fs.writeFileSync('public/chat/chat.css', `* {font-family: 'Yusei Magic', sans-serif;margin: 0;padding: 0;}#chat {height: calc(100vh - 2.5em);overflow-y: scroll;}form {position: fixed;bottom: 0;width: 100%;padding: 0.5em;padding-left: 1em;background: black;}form #id {width: 20%;}form #msg {width: 50%;}form #range {width: 10%;}form #color {width: 10%;}form label {margin-left: 0.5em;color: white;}form label #submit {display: none;}`, 'utf8');
  fs.writeFileSync('public/chat/chat.js', `var io = io();let chat = document.getElementById('chat');let id = document.getElementById('id');let msg = document.getElementById('msg');let submit = document.getElementById('submit');let pic;submit.onclick = e => {if(id.value && msg.value) {id.value.length <= 20 ? io.emit('msg', [id.value, msg.value, range.value, color.value]) : alert('名前は20文字以下にして下さい');msg.value = '';} else alert('メッセージを入力して下さい');return e.preventDefault();};io.on('msg', value => {let div = document.createElement('div');div.textContent = \`<\${value[0]}> \${value[1]}\`;div.style.fontSize = \`\${value[2]}em\`;div.style.color = value[3];chat.appendChild(div);chat.scrollTo(0, chat.scrollHeight);console.log(value[4]);});`, 'utf8');
  fs.writeFileSync('public/chat/index.html', `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Chat</title><link rel="preconnect" href="https://fonts.gstatic.com"><link href="https://fonts.googleapis.com/css2?family=Yusei+Magic&display=swap" rel="stylesheet"><link rel="stylesheet" href="chat.css"><script src="/socket.io/socket.io.js"></script></head><body><div id="chat"></div><form><input id="id" type="text" placeholder="名前" autocomplete="off"><input id="msg" type="text" placeholder="メッセージ" autocomplete="off"><input id="range" type="range" min="1" max="1.5" step="0.1" value="1"><input id="color" type="color"><label>><input id="submit" type="submit"></label></form><script src="chat.js"></script></body></html>`, 'utf8');
  fs.mkdirSync('public/danmaku');
  fs.writeFileSync('public/danmaku/danmaku.js', `let c = document.createElement("canvas");document.body.appendChild(c);let ctx = c.getContext("2d");let t, m;let w = 900;let h = 1600;let k = new Set();resize();addEventListener('resize', resize);addEventListener('mousedown', e => {k.add("mouse");});addEventListener('mouseup', e => {k.delete("mouse");});addEventListener('keydown', e => {k.add(e.key);});addEventListener('keyup', e => {k.delete(e.key);});function resize() {if(innerWidth/9<innerHeight/16) {c.width = innerWidth;c.height = innerWidth/9*16;} else {c.width = innerHeight/16*9;c.height = innerHeight;}m = c.width/w;}let p = new function() {this.x = 450;this.y = 1400;this.hp = 20;}function loop() {t += 1;if(k.has("w")) {p.y -= p.y > 0 ? 5 : 0;}if(k.has("s")) {p.y += p.y < h ? 5 : 0;}if(k.has("a")) {p.x -= p.x > 0 ? 5 : 0;}if(k.has("d")) {p.x += p.x < w ? 5 : 0;}draw();requestAnimationFrame(loop);}function draw() {ctx.fillStyle = "black";ctx.fillRect(0, 0, 900*m, c.height);ctx.fillStyle = "white";ctx.beginPath();ctx.arc(p.x*m, p.y*m, 20*m, 0, Math.PI * 2, true);ctx.fill();}loop();`, 'utf8');
  fs.writeFileSync('public/danmaku/index.html', `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Document</title><style>* { margin: 0; }</style></head><body><script src="danmaku.js"></script></body></html>`, 'utf8');
